rm(list=ls())
###########################################################################################################################
###AIM: estimate transition rates between health states, women, Spain, 2015-2017, wave 6-7 SHARE, health state according to self-rated health
### state 1= excellent/very good/good
### state 2= fair/poor
...
data67 <- read.dta(file="C:\\Users\\Magdalena\\demography\\attrition\\data\\SHARE\\SHARE_6\\sharew6_rel7-1-0_ph.dta", convert.factors = FALSE) %>%
  mutate(self6=ph003_,
         self6=ifelse(self6>0 & self6<4,1,self6),
         self6=ifelse(self6>3, 2, self6)) %>%
  select(mergeid,self6) %>%
  ...
  left_join(read.dta(file="C:\\Users\\Magdalena\\demography\\attrition\\data\\SHARE\\SHARE_7\\sharew7_rel7-1-0_ph.dta", convert.factors = FALSE) %>%
              mutate(self7=ph003_,
                     self7=ifelse(self7>0 & self7<4,1,self7),
                     self7=ifelse(self7>3, 2, self7)) %>%
              select(mergeid,self7)) %>%
...
forimach <- data67 %>%
  mutate(weight6 = weight6/100000,
         across(starts_with("self"), ~ replace_na(., -1))) %>%
...
countries <- sort(unique(forimach$cname))
sex <- c("male","female")
i=12
j=2
...
forimach2 <- forimach %>%
      ...
      select(weight6, birth, death, int1, self6, int2, self7)
    rownames(forimach2) <- 1:nrow(forimach2)
    setwd("C:\\Users\\Magdalena\\demography\\healthmicrosim\\data\\sr")
    write.table(forimach2, file=paste("sr",countries[i],sex[j],"srdata.txt",sep=""), col.names=FALSE,quote=FALSE)
    system2("C:\\Program Files\\IMaCh-0.99r42\\bin\\IMaCh.exe", paste("C:\\Users\\Magdalena\\demography\\healthmicrosim\\data\\sr\\sr",countries[i],sex[j],".txt",sep=""),wait=FALSE)

###################################################probabilities of transition between health states
pij <- read.table(file=paste("C:\\Users\\Magdalena\\demography\\healthmicrosim\\data\\sr\\sr",countries[i],sex[j],"\\PROB_rsr",countries[i],sex[j],".txt",sep=""),sep=" ",skip = 6) %>%
      select(V1,V3,V7,V11,V15,V19,V23)
    colnames(pij) <- c("Age1","p11", "p12", "p13", "p21", "p22", "p23")
    
setwd("C:\\Users\\Magdalena\\demography\\healthmicrosim\\data\\sr\\outprob")
write.table(pij, file=paste(countries[i],sex[j],"sr.txt",sep=""),sep=",",row.names=FALSE)
