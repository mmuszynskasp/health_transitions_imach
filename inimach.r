...
###the issue with the data is that the episodes for the same individuals are repeated in datasets, so a distinct observations need to be selected, 
### in case of each longitudinal database those are different year observations
...
episodes <- function(mydata,startyear){
  endyear <- startyear+1
  
  q1 <- paste0("quartint",substr(startyear,3,4))
  y1 <- paste0("yearint",substr(startyear,3,4))
  weight1 <- paste0("weight",substr(startyear,3,4))
  G1 <- paste0("GALI",substr(startyear,3,4))
  
  q2 <- paste0("quartint",substr(endyear,3,4))
  y2 <- paste0("yearint",substr(endyear,3,4))
  G2 <- paste0("GALI",substr(endyear,3,4))
  
 episode12 <- mydata %>%
    filter(!!as.symbol(y1)==startyear) %>%
    left_join(mydata %>%
                filter(!!as.symbol(y1)==startyear) %>%
    group_by(sex) %>%
    dplyr::summarize(sumweights=sum(!!as.symbol(weight1)))) %>%
    mutate(weightnew=100*!!as.symbol(weight1)/sumweights,
           yearint1=!!as.symbol(y1), 
           quartint1=!!as.symbol(q1), 
           GALI1=!!as.symbol(G1), 
           yearint2=!!as.symbol(y2), 
           quartint2=!!as.symbol(q2), 
           GALI2=!!as.symbol(G2),
           ...
           ageint2 = floor(((yearint2-yearbirth)*4+(quartint2-quartbirth))/4),
           agedied = floor(((yeardied-yearbirth)*4+(quarterdied-quartbirth))/4),
           ageint2=ifelse(is.na(ageint2),999,ageint2),
           GALI2=ifelse((ageint2>=agedied & !is.na(agedied)),0,GALI2),
           yearint2=ifelse(GALI2==0,yeardied,yearint2),
           quartint2=ifelse(GALI2==0,quarterdied,quartint2))%>% 
    select(c(iid,quarterdied,yeardied,sex:quartbirth, weightnew:ageint1,agedied))
  return(episode12)
 }
  
indivall <- episodes(mydata=mydata,startyear=2015) %>%
  add_row(episodes(mydata=mydata,startyear=2016)) %>%
  add_row(episodes(mydata=mydata,startyear=2017)) %>%
  add_row(episodes(mydata=mydata,startyear=2018)) %>%  
  mutate(agedied=ifelse(GALI2==0,agedied,NA),
...
#adjustment to deaths from HMD and prepare data for imach
forimach <- indivall %>%
 filter(sex==1) %>%
 left_join(calibrmarg %>% 
             select(ageint5,wmulti), by="ageint5") %>%
  mutate(weightnew=ifelse(GALI2==3,weightnew*wmulti/4.6,weightnew)) %>% #the observed exposure is in quarters and the models are estimated in months, hence we loose for sure the first 1.5 and last 1.5 months of time in the models
